...
class NuscSequentialDataset(Dataset):
    def __init__(self, cfg, split):
        ...
        self.nusc = NuScenes(dataroot=str(self.data_dir), version=self.version)

        self.n_past_steps = self.cfg["MODEL"]["N_PAST_STEPS"]  # use how many past scans, default = 10
        self.dt_data = self.cfg["DATASET"]["NUSC"]["DELTA_T_DATA"]  # minimum time resolution of lidar scans
    ...
    def __getitem__(self, sample_idx):  # define how to load each sample
        ...
        for ref_time_idx, lidar_token in enumerate(lidar_tokens):
            ...
            if self.transform:
                ...
                # 0 - 9, delta_t = 0.05s
                rela_timestamp = -round(ref_time_idx * self.dt_data, 3)  # -0.9, 0.8 ... 0.0 (relative timestamps)
